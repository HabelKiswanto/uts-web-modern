...
def has_fine(cursor, nim):
    query = f"SELECT fine FROM students WHERE nim = {nim}"
    cursor.execute(query)
...
def book_room(cursor, nim, booking_date, booking_time_str, end_time_str, room_name):
    ...
    if has_fine(cursor, nim):
        ...
        query = f"INSERT INTO room_bookings (student_id, booking_date, booking_time, end_time, room_name) VALUES ({nim}, '{booking_date}', '{booking_time}', '{end_time}', '{room_name}')"
        cursor.execute(query)
...
result = book_room(cursor, nim, booking_date, booking_time, end_time, room_name)
print(result)
